...
require_once 'view/theme/ash/php/PHPColors/Color.php';
...
$scheme           = DI::config()->get('ash', 'scheme', DI::config()->get('ash', 'schema'));
$scheme_accent    = DI::config()->get('ash', 'scheme_accent')    ?: ash_SCHEME_ACCENT_BLUE;
$nav_bg           = DI::config()->get('ash', 'nav_bg')           ?: '#708fa0';
$nav_icon_color   = DI::config()->get('ash', 'nav_icon_color')   ?: '#ffffff';
$link_color       = DI::config()->get('ash', 'link_color')       ?: '#6fdbe8';
$background_color = DI::config()->get('ash', 'background_color') ?: '#ededed';
$contentbg_transp = DI::config()->get('ash', 'contentbg_transp') ?? 100;
$background_image = DI::config()->get('ash', 'background_image') ?: 'img/none.png';
$bg_image_option  = DI::config()->get('ash', 'bg_image_option')  ?: '';
$login_bg_image   = DI::config()->get('ash', 'login_bg_image')   ?: '';
$login_bg_color   = DI::config()->get('ash', 'login_bg_color')   ?: '';
$modified         = DI::config()->get('ash', 'css_modified')     ?: time();
...
$uid = $_REQUEST['puid'] ?? 0;
if ($uid) {
	DI::pConfig()->load($uid, 'ash');

	// Only override display settings that have actually been set
	$scheme           = DI::pConfig()->get($uid, 'ash', 'scheme', DI::pConfig()->get($uid, 'ash', 'schema')) ?: $scheme;
	$scheme_accent    = DI::pConfig()->get($uid, 'ash', 'scheme_accent')    ?: $scheme_accent;
	$nav_bg           = DI::pConfig()->get($uid, 'ash', 'nav_bg')           ?: $nav_bg;
	$nav_icon_color   = DI::pConfig()->get($uid, 'ash', 'nav_icon_color')   ?: $nav_icon_color;
	$link_color       = DI::pConfig()->get($uid, 'ash', 'link_color')       ?: $link_color;
	$background_color = DI::pConfig()->get($uid, 'ash', 'background_color') ?: $background_color;
	$contentbg_transp = DI::pConfig()->get($uid, 'ash', 'contentbg_transp') ?? $contentbg_transp;
	$background_image = DI::pConfig()->get($uid, 'ash', 'background_image') ?: $background_image;
	$bg_image_option  = DI::pConfig()->get($uid, 'ash', 'bg_image_option')  ?: $bg_image_option;
	$modified         = DI::pConfig()->get($uid, 'ash', 'css_modified')     ?: $modified;
}
...
if (($scheme) && ($scheme != '---')) {
	if (file_exists('view/theme/ash/scheme/' . $scheme . '.php')) {
		$schemefile = 'view/theme/ash/scheme/' . $scheme . '.php';
		require_once $schemefile;
	}
	if (file_exists('view/theme/ash/scheme/' . $scheme . '.css')) {
		$schemecssfile = 'view/theme/ash/scheme/' . $scheme . '.css';
	}
}
...
if (!$scheme) {
	if (file_exists('view/theme/ash/scheme/default.php')) {
		$schemefile = 'view/theme/ash/scheme/default.php';
		require_once $schemefile;
	}
	if (file_exists('view/theme/ash/scheme/default.css')) {
		$schemecssfile = 'view/theme/ash/scheme/default.css';
	}
}
...
$css_tpl = file_get_contents('view/theme/ash/css/style.css');
