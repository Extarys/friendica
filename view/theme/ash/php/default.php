...
require_once 'view/theme/ash/php/ash_boot.php';
...
<?php
		...
		// It makes mobile Chrome UI match ash's top bar color.
		$uid = Profile::getThemeUid($a);
		$scheme = DI::pConfig()->get($uid, 'ash', 'scheme', DI::pConfig()->get($uid, 'ash', 'schema'));
		if ($scheme && is_string($scheme) && $scheme != '---') {
			if (file_exists('view/theme/ash/scheme/' . $scheme . '.php')) {
				...
				$scheme_accent =
					DI::pConfig()->get($uid, 'ash', 'scheme_accent') ?:
						DI::config()->get('ash', 'scheme_accent') ?: ash_SCHEME_ACCENT_BLUE;

				require_once $schemefile;
			}
		}

		$nav_bg = $nav_bg ?? DI::pConfig()->get($uid, 'ash', 'nav_bg', DI::config()->get('ash', 'nav_bg', '#708fa0'));
