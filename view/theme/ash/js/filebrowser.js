 ...
 *  the ash theme.and bootstrap modals
 *
...
var FileBrowser = {
	...
	init: function (nickname, type, hash) {
		...
		$(".fbrowser").on("click", ".fbswitcher .btn", function (e) {
			...
			url = baseurl + "/fbrowser/" + FileBrowser.type + "?mode=none&theme=ash";
		...
		if ($("#upload-file").length) {
			...
			var file_uploader = new window.AjaxUpload("upload-file", {
				...
				onComplete: function (file, response) {
					...
					var url = baseurl + "/fbrowser/" + FileBrowser.type + "?mode=none&theme=ash";
					// Load new content to fbrowser window
					FileBrowser.loadContent(url);
				},
			});
