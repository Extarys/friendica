				...
				{{foreach $users as $u}}
					...
					<tr id="user-{{$u.uid}}-detail" class=" details hidden {{if $u.blocked != 0}}blocked{{/if}}">
						...
						<td colspan="4">
							{{if $order_users != $th_users.2.1}}
								<p>
									<a href="{{$baseurl}}/admin/users/blocked?o={{if $order_direction_users == "+"}}-{{/if}}{{$th_users.2.1}}"
							...
							{{if $order_users != $th_users.3.1}}
								<p>
									<a href="{{$baseurl}}/admin/users/blocked?o={{if $order_direction_users == "+"}}-{{/if}}{{$th_users.3.1}}"
										class="btn-link table-order">
							...
							{{if $order_users != $th_users.4.1}}
								<p>
									<a href="{{$baseurl}}/admin/users/blocked?o={{if $order_direction_users == "+"}}-{{/if}}{{$th_users.4.1}}"
										class="btn-link table-order">
							...
							{{if in_array($order_users,[$th_users.2.1, $th_users.3.1, $th_users.4.1]) }}
								<p>
									<a href="{{$baseurl}}/admin/users/blocked?o={{if $order_direction_users == "+"}}-{{/if}}{{$th_users.5.1}}"
										class="btn-link table-order">
										&#8597; {{$th_users.5.0}}</a> :
							...
							<td class="text-right">
								{{if $u.is_deletable}}
									{{if $u.blocked}}
										<a href="{{$baseurl}}/admin/users/blocked/unblock/{{$u.uid}}?t={{$form_security_token}}"
											class="admin-settings-action-link" title="{{$unblock}}">
											<i class="fa fa-check-circle-o" aria-hidden="true"></i>
										</a>
									{{/if}}
									<a href="{{$baseurl}}/admin/users/blocked/delete/{{$u.uid}}?t={{$form_security_token}}"
