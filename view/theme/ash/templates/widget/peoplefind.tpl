<div id="peoplefind-sidebar" class="widget">
	...
	<form action="dirfind" method="get">
		...
		<div class="form-group form-group-search">
			<input id="side-peoplefind-url" class="search-input form-control form-search" type="text" name="search"
				data-bs-toggle="tooltip" title="{{$nv.hint}}" />
			<button id="side-peoplefind-submit" class="btn btn-default btn-sm form-button-search"
				type="submit">{{$nv.findthem}}</button>
		</div>
	</form>
	...
	{{if $nv.inv}}
		<div class="side-link" id="side-invite-link"><button type="button" class="btn-link"
				onclick="addToModal('invite'); return false;">{{$nv.inv}}</button></div>
	{{/if}}
</div>