{{* we have modified the navmenu (look at function ash_remote_nav() ) to have remote links. $nav.userinfo is a new variable and replaces the original $userinfo variable *}}
{{if $nav.userinfo}}
	...
	<nav id="topbar-first" class="navbar navbar-light bg-light fixed-top navbar-expand-lg">
				...
				<ul class="nav">
					...
					{{if $nav.search}}
						<li id="search-box" class="hidden-xs">
							...
							<form class="d-flex" role="search" method="get" action="{{$nav.search.0}}">
								<label class="visually-hidden" for="nav-search-input-field">{{$nav.search.1}}</label>
								<input accesskey="s" id="nav-search-input-field" class="form-control form-control-sm me-2"
									type="search" placeholder="Search" aria-label="Search" title="{{$search_hint}}"
					...
					{{if $nav.userinfo}}
						<li id="nav-user-linkmenu" class="dropdown btn-group account nav-link hidden-xs p-0">
							<div class="btn-group">
								<button type="button" class="btn btn-danger py-0">
									<img id="avatar" class="rounded" style="width: 32px; height: 32px;"
										src="{{$nav.userinfo.icon}}" alt="{{$nav.userinfo.name}}"> {{$nav.userinfo.name}}
								</button>
								...
								{{* <button type="button" accesskey="u" id="main-menu" class="btn btn-link dropdown-toggle"
								data-bs-toggle="dropdown" aria-expanded="false" aria-haspopup="true" aria-expanded="false"
								aria-controls="nav-user-menu">
								<img id="avatar" class="rounded float-start" style="width: 32px; height: 32px;"
									src="{{$nav.userinfo.icon}}" alt="{{$nav.userinfo.name}}">

						<div aria-hidden="true" class="user-title hidden-sm hidden-md">
						...
						<span class="caret float-end"></span>
								...
								<ul id="nav-user-menu" class="dropdown-menu" role="menu" aria-labelledby="main-menu">
									{{if $nav.remote}}
										{{if $nav.sitename}}
									...
									{{foreach $nav.usermenu as $usermenu}}
										<li role="presentation">
											<a role="menuitem" class="dropdown-item {{$usermenu.2}}" href="{{$usermenu.0}}"
												title="{{$usermenu.3}}">
												{{$usermenu.1}}
											</a>
										</li>
									...
									{{if $nav.notifications}}
										<li role="presentation">
											<a role="menuitem" class="dropdown-item" href="{{$nav.notifications.all.0}}"
												title="{{$nav.notifications.1}}">
									...
									{{if $nav.messages}}
										<li role="presentation"><a role="menuitem"
												class="dropdown-item nav-commlink {{$nav.messages.2}} {{$sel.messages}}"
									...
									<li role="presentation" class="divider"></li>
									{{if $nav.contacts}}
										<li role="presentation"><a role="menuitem" id="nav-link-contacts-link"
												class="dropdown-item nav-link {{$nav.contacts.2}}" href="{{$nav.contacts.0}}"
									...
									{{if $nav.delegation}}
										<li role="presentation">
											<a role="menuitem" id="nav-delegation-link"
												class="dropdown-item nav-commlink {{$nav.delegation.2}} {{$sel.delegation}}"
									...
									{{/if}}
									<li role="presentation">
										<a role="menuitem" id="nav-directory-link" class="dropdown-item {{$nav.directory.2}}"
									...
									{{if $nav.apps}}
										<li role="presentation">
											<a role="menuitem" id="nav-apps-link" class="dropdown-item {{$nav.apps.2}}"
									...
									{{if $nav.help}}
										<li role="presentation">
											<a role="menuitem" id="nav-help-link" class="dropdown-item {{$nav.help.2}}"
									...
									{{if $nav.settings}}
										<li role="presentation">
											<a role="menuitem" id="nav-settings-link" class="dropdown-item {{$nav.settings.2}}"
									...
									{{if $nav.admin}}
										<li role="presentation"><a accesskey="a" role="menuitem" id="nav-admin-link"
												class="dropdown-item {{$nav.admin.2}}" href="{{$nav.admin.0}}"
												title="{{$nav.admin.3}}"><i class="fa fa-user-secret fa-fw" aria-hidden="true"></i>
												{{$nav.admin.1}}</a></li>
									{{/if}}
									{{if $nav.tos}}
										<li role="presentation" class="divider"></li>
										<li role="presentation"><a role="menuitem" id="nav-tos-link"
												class="dropdown-item {{$nav.tos.2}}" href="{{$nav.tos.0}}" title="{{$nav.tos.3}}"><i
													class="fa fa-file-text" aria-hidden="true"></i> {{$nav.tos.1}}</a></li>
									{{/if}}
									<li role="presentation" class="divider"></li>
									{{if $nav.logout}}
										<li role="presentation"><a role="menuitem" id="nav-logout-link"
												class="dropdown-item {{$nav.logout.2}}" href="{{$nav.logout.0}}"
									...
									{{else}}
										<li role="presentation"><a role="menuitem" id="nav-login-link"
												class="dropdown-item nav-login-link {{$nav.login.2}}" href="{{$nav.login.0}}"
