<fieldset data-id="{{$profile_field.id}}">
	...
	<p>
		<a id="settings-default-perms-menu" class="settings-default-perms" data-toggle="modal"
			data-target="#profile-field-acl-{{$profile_field.id}}">{{$profile_field.permissions}}
			{{$profile_field.permdesc}}</a>
	</p>

	{{* We include the aclModal directly into the template since we cant use ash's default modal *}}
